	...
	existingUser(username,function(isExists){
		...
		bcrypt.genSalt(10,function(err,salt) {
			bcrypt.hash(password1, salt, function(err, hashword) {		
				//Save the user:
				db.saveUser({username:username,hashword:hashword,email:email,joined:Date.now(),rating:1400,kfactor:30,played:0,won:0,lost:0,drawn:0,rated:0,rwon:0,rlost:0,rdrawn:0,wonpct:0,lostpct:0,drawnpct:0,rwonpct:0,rlostpct:0,rdrawnpct:0},function(err,isValid,id) {
					if(isValid) {
