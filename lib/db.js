...
module.exports.findGameById = function (gameId, callback) {
  onConnect(function (err, connection) {
    r.db(dbConfig['db']).table('games').get(gameId).run(connection, function(err, result) {
      if(err) {
...
module.exports.saveGame = function (game, callback) {  
  onConnect(function (err, connection) {
    r.db(dbConfig.db).table('games').insert(game,{upsert:true}).run(connection, function(err, result) {
      if(err) {
        logerror("[ERROR][%s][saveUser] %s:%s\n%s", connection['_id'], err.name, err.msg, err.message);
        callback(err);
      }
