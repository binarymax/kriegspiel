...
kriegspiel.prototype.pawncaptures = function() {
	...
	for(var i=0,l=moves.length;i<l;i++) {
		var move = moves[i];
		if (move.piece==='p' && (move.flags.indexOf('e')>-1 || move.flags.indexOf('c')>-1)) possible++;
	}
	if(possible>0) {
...
kriegspiel.prototype.offerdraw = function(){
	...
	var message = isactive + " has offered a draw.";
	self.message({type:'offerdraw',isactive:isactive,inactive:inactive,message:message},{action:'start'},{action:'stop'});
};
