...
kriegspiel.prototype.move = function(source,target,scratch) {
	...
	if (result === null) {
		...
		if (gameover!==null) {
			//Game over!
			self.message({type:'gameover',isactive:isactive,inactive:inactive,gameover:gameover},{action:'stop'},{action:'stop'});
			self.finish();
		} else if (check!==null) {
...
kriegspiel.prototype.ispawncapture = function(color,move) {
	...
	return (
		((srank + rdiff) === trank) && //one rank difference
		(Math.abs(sfile-tfile) === 1) //one file difference
	) ? true:false;
};
