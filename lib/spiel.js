...
var replydraw = Spiel.replydraw = function(gameid,answer,player) {
	find(gameid,function(game) {
...
kriegspiel.prototype.start = function(color) {
	...
	if (player && player.username && opponent && opponent.username) {
		...
		self.board.header('White',self.white.username,'Black',self.black.username,'Date',self.startdate.toUTCString());
		self.state = states.active;
	} 

	if (player.socket && player.socket.emit) {
		player.socket.emit('kriegspiel', { color: color, game:self.serialize(color) });
		self.message({type:'welcome',isactive:isactive,inactive:inactive,username:player.username,color:color},{},{});
	}
};
...
kriegspiel.prototype.gameover = function(isactive,inactive) {
	...
	if (board.insufficient_material()) result = {type:'draw',message:'draw_by_insufficient_material',white:0.5,black:0.5};
	if (board.in_threefold_repetition()) result = {type:'draw',message:'draw_by_repetition',white:0.5,black:0.5};
	if (board.in_draw()) result = {type:'draw',message:'draw',white:0.5,black:0.5};
...
//Player offers draw
kriegspiel.prototype.offerdraw = function(){
	var self = this;
	if(!self.okplay) return false;
	var turn = self.board.turn();
	var isactive = colors[turn];
	...
	self.message({type:'offerdraw',isactive:isactive,inactive:inactive},{action:'start'},{action:'stop'});
...
kriegspiel.prototype.acceptdraw = function(){
	var self = this;
	if(!self.okplay) return false;
	var turn = self.board.turn();
	var isactive = colors[turn];
	...
	self.result = {type:'draw',message:'draw_agreed',white:0.5,black:0.5};
	var message1 = inactive + " has accepted the draw!";
	var message2 = "The game is over and has ended in a draw.";
	self.message({type:'acceptdraw',isactive:isactive,inactive:inactive,message:message1},{action:'stop'},{action:'stop'});
...
kriegspiel.prototype.declinedraw = function(){
	var self = this;
	if(!self.okplay) return false;
	var turn = self.board.turn();
	var isactive = colors[turn];
	var inactive = turn==='w'?colors['b']:colors['w'];
	self.drawoffered = false;
	var message = inactive + " has declined the draw offer.";
	self.message({type:'declinedraw',isactive:isactive,inactive:inactive,message:message},{action:'start'},{action:'stop'});
};


//Player Resignation
kriegspiel.prototype.resign = function(){
	var self = this;
	if(!self.okplay) return false;
	var turn = self.board.turn();
	var isactive = colors[turn];
	...
	var message = isactive + " has resigned.";
	self.message({type:'gameover',isactive:isactive,inactive:inactive,message:message},{action:'stop'},{action:'stop'});
	self.finish();
