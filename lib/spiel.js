...
var getGame = function(gameid,callback) {
	...
	if(spiels[gameid]) {
		...
		db.findGameById(gameid,function(err,record){
			//Create the game
			var game = spiels[gameid] = new kriegspiel();
...
var join = Spiel.join = function(gameid,player) {
	getGame(gameid,function(game) {
		//Join the game
		game.join(player,'white',function(err,color){
			if (game.white.username && game.white.socket && game.black.username && game.black.socket) {
...
var init = Spiel.init = function(gameid) {
	var spiel = spiels[gameid];
	if (spiel.white.username && spiel.white.socket && spiel.black.username && spiel.black.socket) spiel.start();
};


var move = Spiel.move = function(gameid,source,target,player) {
	getGame(gameid,function(game) {
		game.join(player,'white',function(err,color){
...
var kriegspiel = function(variant) {
	...
	this.white = { board:new chess.Chess() };
	this.black = { board:new chess.Chess() };
	
	//Remove all opposing pieces on own boards.  These boards are used to test impossible moves.
	for(var f=0,wsq,bsq;f<8;f++) {
		wsq = files[f] + '1'; if(wsq!='e1') this.black.board.remove(wsq);
		bsq = files[f] + '8'; if(bsq!='e8') this.white.board.remove(bsq);
		wsq = files[f] + '2'; this.black.board.remove(wsq);
		bsq = files[f] + '7'; this.white.board.remove(bsq);
	}
...
kriegspiel.prototype.save = function() {
	var self = this;
	db.saveGame(self.serialize(),function(err,success,id){
		if(!err && success && id) self.dbid = id;
	});
};

kriegspiel.prototype.serialize = function() {
	var self = this;
	return {
		gameid:self.id,
		state:self.state,
...
kriegspiel.prototype.message = function(color,message) {
	//this.messages.push({who:who,message:message});
};
...
kriegspiel.prototype.impossible = function(player,move) {
	var board = player.board;
	var fen   = board.fen();
	var ok    = board.move(move)===null?true:false;
	board.load(fen);
	return ok;	
};
...
kriegspiel.prototype.move = function(source,target) {
	...
	if (result === null) {
		if (self.impossible(player,move)) {
			var message = isactive + " has attempted an impossible move";
			player.socket.emit('impossible', { type: 'start', who: isactive, square: target, message:message });
			opponent.socket.emit('impossible', { type: 'stop', who: isactive, square: target, message:message });			
		} else {
		...
		if (gameover!==null) {
			...
			player.socket.emit('gameover', { type: 'stop', who: isactive, message:message });
			opponent.socket.emit('gameover', { type: 'stop', who: isactive, message:message });			
		}
	}
};
