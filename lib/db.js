...
module.exports.findGamesByPlayerAndState = function (username, state, map, callback) {
  onConnect(function (err, connection) {
    r.db(dbConfig['db']).table('games').filter(function(game){ return game('state').eq(state).and(game('whiteusername').eq(username).or(game('blackusername').eq(username))); }).pluck('gameid','state','blackusername','whiteusername','moves','result','rated','turn','startdate','enddate').orderBy(r.desc('enddate')).run(connection, function(err, result) {
      if(err) {
        logerror("[ERROR][%s][findGamesByPlayer] %s:%s\n%s", connection['_id'], err.name, err.msg, err.message);
        callback(null, null);
      }
